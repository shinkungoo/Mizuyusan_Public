/** This file is created by Shizumu on 2022/4/27.
...
namespace AVLTreeClass {
    ...
    typename AVLTree<T>::AVLTreeNode *AVLTree<T>::_clone(const AVLTreeNode *root) {
        ...
        if (new_root->get_left_child()) {
            new_root->get_left_child()->set_parent(new_root);
        }
        new_root->set_right_child(_clone(root->get_right_child()));
        if (new_root->get_right_child()) {
            new_root->get_right_child()->set_parent(new_root);
        }
    ...
    template
    class AVLTree<TelephoneNumber>;
}

/**the implementation of tSet**/
/// basic manipulation of set
template<typename T>
AVLTreeClass::AVLTree<T> tSet<T>::get_data() const {
    return _data;
}
...
void tSet<T>::print() {
    std::cout << "{";
    if (_data.get_size() != 0) {
        for (auto i = _data.get_begin(); i != _data.get_end(); ++i) {
            std::cout << i.get_current_ptr()->get_element() << ","[i == _data.get_last()];
        }
    }
    std::cout << "}";
}
...
tSet<T> tSet<T>::setUnion(const tSet<T> &s) const {
    ...
    for (auto i = s.get_data().get_begin(); i != s.get_data().get_end(); ++i) {
        ret.insert(i.get_current_ptr()->get_element());
    }
    return ret;
}
...
tSet<T> tSet<T>::setIntersection(const tSet<T> &s) const {
    ...
    for (auto i = s.get_data().get_begin(); i != s.get_data().get_end(); ++i) {
        if (this->find(i.get_current_ptr()->get_element())) {
            ret.insert(i.get_current_ptr()->get_element());
        }
    }
...
tSet<T> tSet<T>::setDifference(const tSet<T> &s) const {
    tSet<T> ret{max(this->capacity(), s.capacity())};
    for (auto i = this->get_data().get_begin(); i != this->get_data().get_end(); ++i) {
        if (!s.find(i.get_current_ptr()->get_element())) {
            ret.insert(i.get_current_ptr()->get_element());
        }
    }
...
bool tSet<T>::isSuperset(const tSet<T> &s) const {
    ...
    if (s.empty()) {
        ...
        for (auto i = this->get_data().get_begin(); i != s.get_data().get_end(); ++i) {
            if (!s.find(i.get_current_ptr()->get_element())) {
                ret = false;
                break;
            }
...
std::ostream &operator<<(std::ostream &os, const tSet<T> &s) {
    os << "{";
    if (s.get_data().get_size() != 0) {
        for (auto i = s.get_data().get_begin(); i != s.get_data().get_end(); ++i) {
            os << i.get_current_ptr()->get_element() << ","[i == s.get_data().get_last()];
        }
    }
    os << "}";
...
/// the instantiation of template( or will occur compile error)
template
class tSet<TelephoneNumber>;

template std::ostream &operator<<(std::ostream &os, const tSet<TelephoneNumber> &s);
