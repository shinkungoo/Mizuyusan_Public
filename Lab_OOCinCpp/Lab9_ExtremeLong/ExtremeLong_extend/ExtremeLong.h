...
public:
    T item() const;
    UtilityNumber();
    explicit UtilityNumber(T x);
    ReturnNumber<T> operator*();
    UtilityNumber<T>& operator=(const UtilityNumber<T> & uNumber);
    UtilityNumber<T>& operator=(const T & uNumber);

    template<typename t>
    friend std::istream & operator>>(std::istream &is, UtilityNumber<t> &x);

    template<typename t>
    friend std::ostream & operator<<(std::ostream &is, const UtilityNumber<t> &x);
};
