...
ExtremeLong::digits() const {
    integer minDigits = static_cast<integer>(std::floor((_bitLength_- 1)  * std::log10(2.0))) + 1;
    integer maxDigits = static_cast<integer>(std::floor(_bitLength_ * std::log10(2.0))) + 1;
    if(minDigits == maxDigits){
        return minDigits;
    }else{
        auto tempString = this->toString();
        if(tempString[0] == '-'){
            return tempString.size() - 1;
        }else{
...
ExtremeLong::toString() const{
    ...
    if(_mag_.size() == 1){
        number = std::move(std::to_string(static_cast<uint32_t>(_mag_[0])));
    }else{
        auto temp = *this;
